...
from modulus.models.import_keras import keras as keras_fn
from modulus.models.templates.alexnet import AlexNet
from modulus.models.templates.conv_gru_2d import ConvGRU2D
from modulus.models.templates.helnet import HelNet
from modulus.models.templates.qdq_layer import QDQ
from modulus.models.templates.quantized_conv2d import QuantizedConv2D
from modulus.models.templates.resnet import ResNet
import modulus.models.templates.utils
import modulus.pruning

from modulus.pruning.utils import find_prunable_parent
import modulus.utils
...
class TestPruning(object):
    ...
    def common(self,
        ...
        pruned_model = modulus.pruning.prune(
            model,
            method,
            normalizer,
            criterion,
    ...
    @pytest.mark.parametrize(
        ...
        "granularity, min_num_filters, threshold, dont_prune_elmtwise_input", [
            ...
            (HelNet, 6, 'channels_last', False, (128, 64, 3), "arithmetic_mean",
    ...
    def test_min_weight(self, model, nlayers, data_format, use_batch_norm, input_shape, normalizer,
        ...
        pruned_model = modulus.pruning.prune(model, method, normalizer, criterion, granularity,
    ...
    def test_unknown_params(self, model, nlayers, data_format, use_batch_norm, input_shape,
        ...
        with pytest.raises(NotImplementedError):
            modulus.pruning.prune(model, method, normalizer, criterion, granularity,
                                  min_num_filters, threshold)
    ...
    def test_unsupported_layer(self):
        ...
        with pytest.raises(NotImplementedError):
            modulus.pruning.prune(model, "min_weight", "off", "L2", 8, 1, 0.01)
    ...
    #     with pytest.raises(NotImplementedError):
    #         modulus.pruning.prune(outer_model_layer_before, 'min_weight', 'off', 'L2', 8, 1, 0.01)
    ...
    #     with pytest.raises(NotImplementedError):
    #         modulus.pruning.prune(outer_model_layer_after, 'min_weight', 'off', 'L2', 8, 1, 0.01)
    ...
    def test_with_conv_transpose_head(self, model, nlayers, data_format, use_batch_norm,
        ...
        model = modulus.models.templates.utils.add_deconv_head(
            model=model,
            inputs=inputs,
            nmaps=1,
            upsampling=2,
    ...
    def test_prune_app(self, script_runner, tmpdir, output_fn, model, nlayers, data_format,
        ...
        script = 'modulus-prune'
        # Path adjustment for bazel tests
        if os.path.exists(os.path.join('moduluspy', script)):
            script = os.path.join('moduluspy', script)
