...
import iva.common.no_warning  # noqa pylint: disable=W0611
...
def build_command_line_parser(package_name, modules=None):
    ...
    parser.add_argument(
        "--mpirun-arg",
        type=str,
        default="-x NCCL_IB_HCA=mlx5_4,mlx5_6,mlx5_8,mlx5_10 -x NCCL_SOCKET_IFNAME=^docker0,lo",
...
def launch_job(package, package_name, cl_args=None):
    ...
    import sys
    import iva.X.scripts
    from iva.common.entrypoint import launch_job

    if __name__ == "__main__":
        launch_job(iva.X.scripts, "X", sys.argv[1:])
    """
    ...
    if np > 1:
        ...
        mpi_command = 'mpirun -np {} --oversubscribe --bind-to none --allow-run-as-root'.format(
            np
        )
        if multi_node:
    ...
    task_command = "python3.6 {}".format(modules[task]["runner_path"])
    ...
    try:
        ...
        if e.output is not None:
            print("TLT task: {} failed with error:\n{}".format(task, e.output))
        sys.exit(-1)
