...
def aug_hsv(img, h=0.1, s=1.5, v=1.5):
    ...
    hsv = cv2.cvtColor(np.clip(img, 0, 255).astype(np.uint8), cv2.COLOR_RGB2HSV).astype(float)
    hsv[..., 1:] *= sv_mul
    hsv[..., 0] += (np.random.rand() * 2.0 - 1.0) * h * 180
    hsv = np.round(hsv).astype(np.int)
    hsv[..., 0] %= 180
    hsv[..., 1:] = np.clip(hsv[..., 1:], 0, 255)
    return cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2RGB).astype(float)
